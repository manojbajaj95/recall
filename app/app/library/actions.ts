...
export const createCollection = async (prevState: State, formData: FormData): Promise<State> => {
  ...
  const { data, error } = await client
    .from('collections')
    .insert([
      { collection_name: name, collection_details: description },
    ]).select("id")
  if (error) {
...
export const deleteSource = async (source: number, prevState: State, formData: FormData): Promise<State> => {
  ...
  const { data: deleteData, error: deleteError } = await client.from('sources').delete().eq('id', source).select("id, path")
