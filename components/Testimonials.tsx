      ...
      <Container>
        <div className="mx-auto max-w-2xl md:text-center">
          <h2 className="font-display text-3xl tracking-tight text-slate-900 sm:text-4xl">
            {marketingConfig.testimonials.title}
          </h2>
          <p className="mt-4 text-lg tracking-tight text-slate-700">{marketingConfig.testimonials.subtitle}</p>
        </div>
        <ul
          role="list"
          className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:gap-8 lg:mt-20 lg:max-w-none lg:grid-cols-3"
        >
          {marketingConfig.testimonials.testimonials.map((column, columnIndex) => (
            <li key={columnIndex}>
